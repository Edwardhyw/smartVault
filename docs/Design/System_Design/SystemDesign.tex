...
\midrule
User input & Item description(in) & User inputs on web-page on which item to be found.\\
Displace item position & Item position (output) & Displace item position on web-page for the desired item.\\
...
\end{figure}
\noindent\textbf{Record Initial Condition:} The device will take a picture of the room as the initial position information for each object and save it to the database. \\\\
...
\textbf{Detect Object Movement:} When the device detects the movement of an object that is made by another object (for example, colliding with another object and fall), it will trace the new location of the object. \\\\
...
\textbf{UIR3:} The UI must be able to notify the user when the WI-FI signal is weak or unstable.\\\\
...
\textbf{PAR1:} The time based value used in the device should be precise to minute.\\\\
