...
class Welcome_win:
    def __init__(self, win, title, size, location):
        self.win = win
        self.title = title
        self.size = size
    ...
    def Input(self):      # functions used to take the input from the computer
        ...
        with open(self.location + "user.txt") as f:
            lines = f.readlines()
        if (user == lines[0].split(" ")[0]) and (password == lines[0].split(" ")[1]):
            self.win.destroy()
            det_win = tk.Tk()
            detect_window = Det_Window(det_win, self.location)
