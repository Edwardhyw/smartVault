...
def extract_item(path, orimg1, orimg2):
    ...
    for c in contours:
        ...
        if score2 > score1:
            ...
            item1 = sr.upsample(item1)
            item1 = cv2.resize(item1,dsize=None,fx=6,fy=6)
            before_item.append(item1)

        else:
            #
            #cv2.imwrite(os.path.join(path, "r1.jpg"), item1)
            item2 = sr.upsample(item2)
            item2 = cv2.resize(item2, dsize=None, fx=6, fy=6)
    ...
    for i in range(itemListNum):
        ...
        for j in range(itemNum):
            if item_match(itemList[i],storedItem[j]):
                ...
                frameStorage(directory,j,stored , positionList[i], "u")
                flag =1
        if flag == 0:
            stored = cv2.resize(itemList[i], resize_range)
            frameStorage(directory, itemNum,stored, positionList[i], "i")
